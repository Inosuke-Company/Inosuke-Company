...
const contributionsBaseline = Number(process.env.CONTRIBUTIONS_BASELINE || 538);
const privateCommitsBaseline = Number(process.env.PRIVATE_COMMITS_BASELINE || 523);
const apiTotal = c.contributionCalendar.totalContributions || 0;
const displayedTotal = exactPrivate ? apiTotal : Math.max(apiTotal, contributionsBaseline);
const totalLabel = `${fmt(displayedTotal)}${!exactPrivate && apiTotal < contributionsBaseline ? '+' : ''}`;
...
const metrics = exactPrivate ? [
  ['Contribuições', totalLabel, '#39d353'],
  ['Commits (incl. privados)', fmt(c.totalCommitContributions), '#58a6ff'],
  ...
  ['Contribuições registradas', totalLabel, '#39d353'],
  ['Commits no projeto principal', `${fmt(privateCommitsBaseline)}+`, '#58a6ff'],
  ['Commits públicos atuais', fmt(c.totalCommitContributions), '#79c0ff'],
  ['Atividade privada visível', fmt(privateActivity), '#a371f7'],
  ['Pull Requests públicos', fmt(c.totalPullRequestContributions), '#f0883e'],
  ['Reviews públicas', fmt(c.totalPullRequestReviewContributions), '#56d364']
];

function overviewSvg() {
  const cards = metrics.map((m,i) => {
    const col=i%3,row=Math.floor(i/3),x=45+col*375,y=112+row*92;
    return `<rect x="${x}" y="${y}" width="345" height="72" rx="14" fill="#161b22" stroke="#30363d"/><text x="${x+18}" y="${y+28}" fill="#8b949e" font-family="Arial,sans-serif" font-size="15">${esc(m[0])}</text><text x="${x+18}" y="${y+57}" fill="${m[2]}" font-family="Arial,sans-serif" font-size="26" font-weight="700">${esc(m[1])}</text>`;
  }).join('');
  const foot = exactPrivate
    ? 'Métricas detalhadas privadas autorizadas por GitHub Actions Secret.'
    : 'O projeto principal é privado; o snapshot preserva seu histórico sem revelar nome, código ou dados do repositório.';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="330" viewBox="0 0 1200 330"><defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#0d1117"/><stop offset="1" stop-color="#111827"/></linearGradient></defs><rect width="1200" height="330" rx="22" fill="url(#bg)" stroke="#30363d"/><text x="45" y="55" fill="#58a6ff" font-family="Arial,sans-serif" font-size="30" font-weight="700">Quiroz · GitHub Metrics</text><text x="45" y="84" fill="#8b949e" font-family="Arial,sans-serif" font-size="15">self-hosted · atualizado automaticamente no próprio repositório</text>${cards}<text x="45" y="310" fill="#8b949e" font-family="Arial,sans-serif" font-size="13">${esc(foot)}</text></svg>`;
}
...
console.log({ user, apiTotal, displayedTotal, publicCommits:c.totalCommitContributions, privateActivity, activeDays, exactPrivate, contributionsBaseline, privateCommitsBaseline });
